...
class ItemRepositoryTest {
    ...
    @Test
    @DisplayName("범위 해당하는 테스트")
    public void findByPriceBetweenTest() {
        this.createItemTest();
        List<Item> itemList = itemRepository.findByPriceBetween(10000,10003);
        for(Item item : itemList) {
            System.out.println(item.getItemNm());
        }
    }
    @Test
    @DisplayName("전달된 문자열 검색")
    public void findByItemNmContainingTest() {
        this.createItemTest();
        List<Item> itemList = itemRepository.findByItemNmContaining("6");
        for(Item item : itemList) {
            System.out.println(item.toString());
        }
    }
    @Test
    @DisplayName("@Query를 이용한 검색")
